...
class TeufelTimeDomain():
    """
    This class describes a time-domain optical field recorded
    at a specified observation point during a TEUFEL run.
    It contains 6 components of electric and magnetic fields
    at fixed time steps in laboratory system.
    """
    
    def __init__(trace):
        """
        Create an empty TeufelTimeDomain object.
        """

    @classmethod
    def read(cls, filename, verbose=True, list_parameters=False, list_columns=False):
        """
        Import a TeufelTimeDomain object from an SDDS file.
        """
        # creat an empty object of the class
        trace = cls()
        ...
        if ("t0" in data.parameterName and
            ...
            trace.t0 = data.getParameterValue("t0")
            trace.dt = data.getParameterValue("dt")
            trace.nots = data.getParameterValue("NumberTimeSteps")
            if verbose:
                print(r'%d steps starting at t0=%12.3g s step dt=%12.3g s' % (trace.nots,trace.t0,trace.dt))
            trace.t = (np.linspace(trace.t0, trace.t0+trace.nots*trace.dt, num=trace.nots)+0.5*trace.dt)*1e9
        else:
            print("TeufelTimeDomain : unable to read the timing")
            return None

        if ("t" in data.columnName):
            trace.t = np.array(data.getColumnData("t"))
            if verbose:
                print("using provided time column")
    
        trace.Ex = np.array(data.getColumnData("Ex"))
        trace.Ey = np.array(data.getColumnData("Ey"))
        trace.Ez = np.array(data.getColumnData("Ez"))
        trace.Bx = np.array(data.getColumnData("Bx"))
        trace.By = np.array(data.getColumnData("By"))
        trace.Bz = np.array(data.getColumnData("Bz"))
...
class TeufelScreen():
    ...
    def read(cls, filename, verbose=True):
        ...
        if hdf.get('Screen', getclass=True) == h5py._hl.dataset.Dataset:
            ...
            if verbose: print("Nx=%d Ny=%d Nots=%d" % (screen.Nx,screen.Ny,screen.nots))
            sd = np.array(sn)
            screen.origin = sd[0]
            if verbose: print("origin= (%g, %g, %g) m" % (screen.origin[0],screen.origin[1],screen.origin[2]))
            screen.normal = sd[1]
            if verbose: print("normal= (%g, %g, %g) m" % (screen.normal[0],screen.normal[1],screen.normal[2]))
            screen.dX = sd[2]
            screen.dx = np.linalg.norm(screen.dX)
            screen.ex = screen.dX / screen.dx
            if verbose: print("dX= (%g, %g, %g) m" % (screen.dX[0],screen.dX[1],screen.dX[2]))
            screen.dY = sd[3]
            screen.dy = np.linalg.norm(screen.dY)
            screen.ey = screen.dY / screen.dy
            if verbose: print("dY= (%g, %g, %g) m" % (screen.dY[0],screen.dY[1],screen.dY[2]))
            screen.t0 = sn.attrs.get('t0')
            screen.dt = sn.attrs.get('dt')
            screen.dtx = sn.attrs.get('dtx')
            screen.dty = sn.attrs.get('dty')
            if verbose: print("t0=%g dt=%g dtx=%g dty=%g" % (screen.t0, screen.dt, screen.dtx, screen.dty))
        else:
    ...
    def totalEnergy(self):
        """
        get the total incident radiation energy [J] recorded on the screen
        """
        sum = 0.0
        for ix in range(self.Nx):
            for iy in range(self.Ny):
                sum += self.totalPowerDensity(ix,iy)
        return sum*self.dx*self.dy
    ...
    def spectralDensityH(self,ix,iy):
        """
        get the spectral power density [J/(m² Hz)] of the horizontal polarization
        """  
        nf = self.nots
        fmax = 1.0/self.dt
    ...
    def spectralDensityV(self,ix,iy):
        """
        get the spectral power density [J/(m² Hz)] of the vertical polarization
        """  
        nf = self.nots
        fmax = 1.0/self.dt
        f = np.linspace(0.0,fmax,nf)[:nf//2]
