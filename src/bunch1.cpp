...
void Bunch::setNOP(int NP) { NOP = NP; };

int Bunch::getNOP() { return NOP; };

int Bunch::getNOTS() { return NT; }

int Bunch::getCharge() { return Charge; }

int Bunch::getMass() { return Mass; }

tuple<Vector, Vector> Bunch::MutualField(int stepnumber, int ParticleID,
                                         double t) {
...
#pragma omp parallel for
  ...
  cout << "\033[1;31m Tracking Completed in: \033[0m" << (end - start)
       << "\033[1;31m seconds\033[0m\n" << endl;
}
...
#pragma parallel for private(t_h, x_h, p_h, gamma_h, beta_h, F, E_h, B_h,      \
                             dp_dt, p_i1, gamma_i1, beta_i1, k, qmt2, qm,      \
                             tstep)
  for (int k = 0; k < NOP; k++) {
...
#pragma parallel for private(t2, i, t_h, x_h, p_h, gamma_h, beta_h, F, E_h,    \
                             B_h, dp_dt, p_i1, gamma_i1, beta_i1, k, qmt2, qm, \
                             tstep)
    for (int k = 0; k < NOP; k++) {
      ...
      gamma_i1[k] =
          sqrt(0.5 * (sigma +
                      sqrt(sigma * sigma + 4.0 * (tau2nd + u_star * u_star))));
      Vector T = tau / gamma_i1[k];
      p_i1[k] = (p_prime + T * dot(p_prime, T) + cross(p_prime, T)) /
                (1.0 + T.abs2nd());
      beta_i1[k] = p_i1[k] / gamma_i1[k];
      int stepnumber = i + 1;
...
int Bunch::WriteSDDSTrajectory() {
  SDDS_DATASET data;
  char buffer[100];
  int Initialize = SDDS_InitializeOutput(&data, SDDS_BINARY, 1, NULL, NULL,
                                         "trajectory.sdds");
  ...
  if (SDDS_DefineSimpleParameter(&data, "NumberOfParticles", NULL,
                                 SDDS_DOUBLE) != 1 ||
      SDDS_DefineSimpleParameter(&data, "TotalTime", "s", SDDS_DOUBLE) != 1 ||
      SDDS_DefineSimpleParameter(&data, "BunchTotalCharge", "C", SDDS_DOUBLE) !=
          1 ||
      SDDS_DefineSimpleParameter(&data, "BunchTotalMass", "kg", SDDS_DOUBLE) !=
          1) {
  ...
  if (SDDS_DefineColumn(&data, "Number\0", "No\0", NULL, "RowNumber\0", NULL,
                        SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "t\0", "t\0", "s\0", "TimeInSeconds\0", NULL,
                        SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "x\0", "x\0", "m\0", "DisplacementInX\0", NULL,
                        SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "y\0", "y\0", "m\0", "DisplacementInY\0", NULL,
                        SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "z\0", "z\0", "m\0", "DisplacementInZ\0", NULL,
                        SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "px\0", "px\0", NULL, "Gammabetax\0", NULL,
                        SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "py\0", "py\0", NULL, "Gammabetay\0", NULL,
                        SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "pz\0", "pz\0", NULL, "Gammabetaz\0", NULL,
                        SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "Ax\0", "ax\0", "msec2\0", "AccelerationInX\0",
                        NULL, SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "Ay\0", "ay\0", "msec2\0", "AccelerationInY\0",
                        NULL, SDDS_DOUBLE, 0) == -1 ||
      SDDS_DefineColumn(&data, "Az\0", "az\0", "msec2\0", "AccelerationInZ\0",
                        NULL, SDDS_DOUBLE, 0) == -1) {
  ...
  for (int32_t i = 0; i < (int32_t)nop; i++) {
    ...
    if (SDDS_SetParameters(&data, SDDS_SET_BY_NAME | SDDS_PASS_BY_VALUE,
                           "NumberOfParticles", nop, "TotalTime", TotalTime,
                           "BunchTotalCharge", Qtot, "BunchTotalMass", Mtot,
                           NULL) != 1)
    ...
    for (int32_t k = 0; k < (int32_t)nots; k++) {
      ...
      if (SDDS_SetRowValues(&data, SDDS_SET_BY_NAME | SDDS_PASS_BY_VALUE, k,
                            "Number", (double)(k), "t",
                            (double)(b[i].TrajTime(k)), "x",
                            (double)((b[i].TrajPoint(k)).x), "y",
                            (double)((b[i].TrajPoint(k)).y), "z",
                            (double)((b[i].TrajPoint(k)).z), "px",
                            (double)((b[i].TrajMomentum(k)).x), "py",
                            (double)((b[i].TrajMomentum(k)).y), "pz",
                            (double)((b[i].TrajMomentum(k)).z), "Ax",
                            (double)((b[i].TrajAcceleration(k)).x), "Ay",
                            (double)((b[i].TrajAcceleration(k)).y), "Az",
                            (double)((b[i].TrajAcceleration(k)).z), NULL) != 1)
...
// write sdds file with time as a parameter. each page has x,y,z,px,py,pz at
// different time steps
int Bunch::WriteSDDSTime() {
  SDDS_DATASET data;
  char buffer[100];
  int Initialize = SDDS_InitializeOutput(&data, SDDS_BINARY, 1, NULL, NULL,
                                         "time-trajectory.sdds");
  double charge, mass, nots, nop;
  charge = (double)Charge;
  mass = (double)Mass;
  nots = (double)NT;
  ...
  if (SDDS_DefineSimpleParameter(&data, "NumberOfParticles", NULL,
                                 SDDS_DOUBLE) != 1 ||
      SDDS_DefineSimpleParameter(&data, "TotalTime", "seconds", SDDS_DOUBLE) !=
          1 ||
      SDDS_DefineSimpleParameter(&data, "BunchTotalCharge", "Coloumbs",
                                 SDDS_DOUBLE) != 1 ||
      SDDS_DefineSimpleParameter(&data, "BunchTotalMass", "kg", SDDS_DOUBLE) !=
          1 ||
  ...
  for (int32_t i = 0; i < (int32_t)nots; i++) {
    ...
    if (SDDS_SetParameters(&data, SDDS_SET_BY_NAME | SDDS_PASS_BY_VALUE,
                           "NumberOfParticles", nop, "TotalTime", TotalTime,
                           "BunchTotalCharge", Qtot, "BunchTotalMass", Mtot,
                           "Time", (double)(b[0].TrajTime(i)), NULL) != 1) {
    ...
    for (int32_t k = 0; k < (int32_t)nop; k++) {
      ...
      if (SDDS_SetRowValues(
              &data, SDDS_SET_BY_NAME | SDDS_PASS_BY_VALUE, k, "Number",
              (double)(k), "x", (double)((b[k].TrajPoint(i)).x), "y",
              (double)((b[k].TrajPoint(i)).y), "z",
              (double)((b[k].TrajPoint(i)).z), "px",
              (double)((b[k].TrajMomentum(i)).x), "py",
              (double)((b[k].TrajMomentum(i)).y), "pz",
              (double)((b[k].TrajMomentum(i)).z), "Ax",
              (double)((b[k].TrajAcceleration(i)).x), "Ay",
              (double)((b[k].TrajAcceleration(i)).y), "Az",
              (double)((b[k].TrajAcceleration(i)).z), NULL) != 1) {
