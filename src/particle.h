...
//! data structure of the particle serialization buffer
struct __attribute__((packed)) ParticleInfo {
    double Charge;
    double Mass;
    double time;
    Vector X;
...
public:
     ...
     *  @TODO Use second order interpolation.
     */
    void CoordinatesAtTime(double time, Vector *position, Vector *momentum);
    
    /*! Find the trajectory point from which a signal propagating with the speed of light
     *  reaches the given observation position at the given time.
     *
     ...
     *  lies within. Then, the correct position within the interval
     *  is found using a Newton-Raphson scheme.
     *
     ...
     *
     *  If the observation point si chosen such that no signal from
     *  the particle trajectory can have reached it std::nullopt is returned
     *
     *  @param[in] obs_time the time of observation
     *  @param[in] obs_pos the position of the observer
     *
     *  @return the particle information at the source point
     *
     *  @throws RANGEexception
     *  An exception is thrown if the particle is not properly initialized.
     */
    std::optional<ParticleInfo> SolveRetardation(double obs_time, Vector obs_pos);
...
private:
    ...
    /*! Compute the time when a signal emitted from the trajectory point
     *  with given index is observed at the ObservationPoint.
     * 
     *  @todo watch out! - validity of the index is not checked
     * 
     *  Needs to be virtual as the teufel.integrate_field example overwrites it.
     */
